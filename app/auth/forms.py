...
from flask import Markup, current_app, flash, request, url_for
from flask_login import current_user
from flask_wtf import FlaskForm as BaseForm
from speaklater import make_lazy_gettext
from wtforms import BooleanField, Field, HiddenField, PasswordField, \
    StringField, SubmitField, ValidationError, validators, IntegerField, \
    FormField, TextAreaField, SelectField, FieldList
from flask_security.confirmable import requires_confirmation
from flask_security.utils import _, _datastore, config_value, get_message, \
    localize_callback, validate_redirect_url, \
    ...
    def __init__(self, *args, **kwargs):
        ...
        if current_app.extensions['security'].recoverable and \
            ...
            html = Markup(u'<a href="{url}">{message}</a>'.format(
                url=url_for('security.forgot_password'),
                message=get_message("FORGOT_PASSWORD")[0],
            ))
            self.password.description = html

    def validate(self):
        print(not super(LoginForm, self).validate())
        if not super(LoginForm, self).validate():
            return False

        user_id = User.query.join(Tenant).filter(func.lower(User.email) == func.lower(self.email.data) and func.lower(Tenant.subdomain) == func.lower(self.subdomain.data)).first().id
        print("**** ****")
        print(user_id)
        if user_id is None:
            self.user is None
        else:
            self.user = _datastore.get_user(user_id)
        print(self.user)
...
class TenantForm(Form):
    tenant_name = StringField('Company Name', [Optional()])
...
class ContactForm(Form):
    ...
    def to_dict(form):
        def is_user_data(member):
            return isinstance(member, Field) and \
                member.name != 'csrf_token'
...
class TenantRegisterForm(ExtendedRegisterForm):
    organization_name = StringField('Company Name', validators=[DataRequired()])
...
class TenantSelectForm(Form):
    organization = QuerySelectField(
                        'Organization',
                        get_label="name")
